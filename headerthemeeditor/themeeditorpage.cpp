    ...
    , mThemeSession(new GrantleeThemeEditor::ThemeSession(projectDir, u"headerthemeeditor"_s))
{
    ...
    mTabWidget->addTab(mEditorPage, i18n("Editor (%1)", u"header.html"_s));
    ...
    mDesktopPage = new GrantleeThemeEditor::DesktopFilePage(u"header.html"_s, opt, this);
    mDesktopPage->setDefaultDesktopName(u"header.desktop"_s);
...
void ThemeEditorPage::slotExtraHeaderDisplayChanged(const QStringList &extraHeaders)
{
    ...
    for (QString var : extraHeaders) {
        var = QLatin1StringView("header.") + var.remove(u'-');
        result << var;
    }
